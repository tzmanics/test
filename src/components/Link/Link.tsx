...
const getIcon = (icon: string) => {
  ...
  return (
    <Icon
      className={`fa${
        brandIcons.includes(icon.toLowerCase()) ? "b" : "s"
      } fa-${icon.toLowerCase()}`}
    />
  );
};
...
const getButtonContent = (
  ...
  return (
    <ButtonWrap
      sx={{
        flexDirection: iconPosition === "Left" ? "row-reverse" : undefined,
      }}
      component="span"
    >
      ...
      {icon && (
        <Box
          component="span"
          sx={{ margin: iconPosition === "Left" ? "0 10px 0 0" : "0 0 0 10px" }}
        >
          {icon && getIcon(icon)}
        </Box>
      )}
...
const Link = React.forwardRef<any, LinkProps>(function Link(props, ref) {
  ...
  const className = clsx(classNameProps, {
    [activeClassName]:
      (router?.asPath === pathname || router?.asPath === `${pathname}/`) &&
      activeClassName,
  });
  ...
  if (!noLinkStyle && icon && !text) {
    ...
    return (
      <IconButton aria-label={icon} onClick={other.onClick} size="large">
        {getIcon(icon)}
      </IconButton>
    );
  }
  ...
  if (variant?.includes("button-")) {
    const buttonVariant = variant.replace("button-", "") as
      | "text"
      | "outlined"
      | "contained"
      | undefined;
